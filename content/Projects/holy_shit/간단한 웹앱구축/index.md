...
node.js로 생성하는 http 서버는 대충 이런 모양입니다. `createServer`에 달아준 콜백함수에 request 파라미터로 http 요청이 들어오고 (`http:// ~~`) request 에 대한 응답을 response 객체를 통해 보내는 형식입니다. express는 여기 콜백함수에 해당합니다.

자 그럼 폴더 구조를 살펴봅시다.

### bin/www
package.json 파일을 열어보면 다음과 같습니다.
```json
{
  "name": "myapp",
  "version": "0.0.0",
...
`node ./bin/www` 는 `bin/www` 파일을 node로 실행하는 명령입니다. 
...
대충 app.js 모듈을 http로 실행하는 내용이 담겨있습니다. 즉 `node ./bin/www` 명령은 app.js 모듈을 http request에 response할 콜백함수로 사용합니다. 또한 3000번 포트로 서버를 실행하기 때문에 3000 포트로 들어오는 http request에 대해 app.js가 응답하겠죠.
...
### public
보통 호스팅할 파일들을 담는 폴더로 사용합니다. app.js를 먼저 살펴봅시다.
...
`index.html`에 `images/welcome.png`,`style.css`,`models.js`가 사용된다고 합시다. 그리고 서버는  `localhost:3000/` 요청을 받으면 `index.html`파일을 호스팅한다고 합시다.
서버를 실행하고 `localhost:3000/` 주소에 접속하게되면 브라우저는 `localhost:3000/` 요청을 서버로 보냅니다. 서버는 `index.html` 파일 내용을 브라우저로 전송하고
브라우저는 받은 내용을 화면에 렌더링합니다. 렌더링된 `index.html`은 `images/welcome.png`,`style.css`,`models.js` 파일이 필요하므로 서버에 다시 이 파일들을 전송해 달라고 요청합니다.
...
설정해주는거죠. 이제 `localhost:3000/images/welcome.png`, `localhost:3000/style.css` 같은 요청엔 `public/images/welcome.png`, `public/style.css`를 응답하게 됩니다. 
...
만약 위와 같이 `./routes/index.js`를 수정한다고 합시다. 그리고 `localhost:3000/users` 요청을 보냈다고 합시다. indexRouter를 usersRouter보다 먼저 달아놨고 `/users`요청은 `/`로 시작하는 요청에 부합하기 때문에 요청은 indexRouter로 들어옵니다. 그리고 새로 추가한 코드가 실행되겠죠. 그럼 usersRouter는 어떻게 될까요. 실행되지 않습니다. 이미 요청에 대한 응답이 끝났기 때문에 그 다음 라우터/미들웨어는 실행되지 않습니다.
...
템플릿 파일은 html에 특정 언어를 덧붙여 프로그래밍이 가능하도록 만든 파일입니다. 대표적으로 JSP가 있죠.(Html + Java)
...
원래 목적은 파일을 온프레미스 환경에서 호스팅 해보는 것 까지가 목적이었는데 글이 너무 길어져서 한번 끊고 가야겠네요.  
다음 글은 login.html과 signup.html을 호스팅하도록 코드를 수정하고 서버를 가동해 보는 내용이 될 것 같습니다.